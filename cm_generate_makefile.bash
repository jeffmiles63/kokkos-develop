...
echo cmake $COMPILER_CMD  -DCMAKE_CXX_FLAGS="${CXXFLAGS}" -DCMAKE_INSTALL_PREFIX=${PREFIX} -DKokkos_DEVICES=$KOKKOS_DEVICES -DKokkos_ARCH=$KOKKOS_ARCH -DKokkos_ENABLE_TESTS=ON -DKokkos_ENABLE_EXAMPLES=${KOKKOS_DO_EXAMPLES} -DKokkos_OPTIONS=${KOKKOS_OPT} -DKokkos_CUDA_OPTIONS=${KOKKOS_CUDA_OPT} -DCMAKE_CXX_COMPILER=${COMPILER} -DCMAKE_VERBOSE_MAKEFILE=ON -DCMAKE_CXX_EXTENSIONS=OFF -DKokkos_CUDA_DIR=${CUDA_PATH} -DKokkos_CXX_STANDARD=${KOKKOS_CXX_STANDARD} ${KOKKOS_PATH} 
