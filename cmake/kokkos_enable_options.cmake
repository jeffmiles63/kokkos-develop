...
KOKKOS_OPTION(CUDA_OPTIONS "" STRING "A list of CUDA specific options to enable" DEFAULT)
IF (Kokkos_CUDA_OPTIONS MATCHES ",")
  MESSAGE(WARNING "-- Detected a comma in: Kokkos_CUDA_OPTIONS=`${Kokkos_CUDA_OPTIONS}`")
  MESSAGE("-- Although we prefer Kokkos_CUDA_OPTIONS to be semicolon-delimited, we do allow")
  MESSAGE("-- comma-delimited values for compatibility with scripts (see github.com/trilinos/Trilinos/issues/2330)")
  STRING(REPLACE "," ";" Kokkos_CUDA_OPTIONS "${Kokkos_CUDA_OPTIONS}")
  MESSAGE("-- Commas were changed to semicolons, now Kokkos_CUDA_OPTIONS=`${Kokkos_CUDA_OPTIONS}`")
ENDIF()

FOREACH(OPT ${Kokkos_CUDA_OPTIONS})
STRING(TOUPPER ${OPT} UC_NAME)
IF (UC_NAME MATCHES "RDC")
   MESSAGE(STATUS "Enabling Cuda RDC from Kokkos_CUDA_OPTIONS")
   KOKKOS_ENABLE_OPTION(CUDA_RELOCATABLE_DEVICE_CODE  ON "Whether to enable relocatable device code (RDC) for CUDA" OVERRIDE)
ENDIF()
IF (UC_NAME MATCHES "FORCE_UVM")
   MESSAGE(STATUS "Enabling Cuda UVM from Kokkos_CUDA_OPTIONS")
   KOKKOS_ENABLE_OPTION(CUDA_UVM             ON "Whether to enable unified virtual memory (UVM) for CUDA" OVERRIDE)
ENDIF()
IF (UC_NAME MATCHES "USE_LDG")
   MESSAGE(STATUS "Enabling Cuda LDG from Kokkos_CUDA_OPTIONS")
   KOKKOS_ENABLE_OPTION(CUDA_LDG_INTRINSIC   ON "Whether to use CUDA LDG intrinsics" OVERRIDE)
ENDIF()
IF (UC_NAME MATCHES "enable_lambda")
   MESSAGE(STATUS "Enabling Cuda lambda from Kokkos_CUDA_OPTIONS")
   KOKKOS_ENABLE_OPTION(CUDA_LAMBDA          ON "Whether to activate experimental laambda features" OVERRIDE)
ENDIF()
ENDFOREACH()
